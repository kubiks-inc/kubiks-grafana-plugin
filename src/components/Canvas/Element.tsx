...
//             {item.label && <div className="text-2xl font-medium text-white/60">{item.label}</div>}
//             <BlocksComponent blocks={item.value?.data as any} router={router} disabled={true} />
//           </div>
//         )
//       case 'links':
//         return (
    ...
    >
        ...
        {!isSimplifiedView && (
          <div className="space-y-8 animate-fadeIn">
            ...
            {nonLinkItems
              ...
              .map((item, i) => <p>Other fields</p>)}

            {/* Blocks */}
            {nonLinkItems
              .filter((item) => item.type === 'blocks')
              .map((item, i) => <p>Blocks</p>)}
          </div>
        )}
      ...
      {!isSimplifiedView && linkItems.length > 0 && (
        <>
          ...
          <div className="animate-fadeIn">{linkItems.map((item, i) => <p>Links</p>)}</div>
        </>
      )}
    </div>
  )
